...
with st.sidebar:
    st.markdown("---")
    st.markdown("### 🕵️‍♂️ 管理員專區")
    # 輸入密碼才能看到名單
    admin_pwd = st.text_input("輸入密碼查看名單", type="password")
    
    if admin_pwd == "1234":  # 👉 您的密碼在這裡，可自行修改
        st.success("✅ 登入成功")
        if os.path.exists("leads.csv"):
            ...
            st.download_button(
                label="📥 下載名單備份 (CSV)",
                data=csv_data,
                file_name="client_emails.csv",
                mime="text/csv",
                type="primary"
            )
        else:
            st.warning("目前還沒有人留資料喔！")
    elif admin_pwd != "":
        st.error("密碼錯誤")
...
if cross_point:
    ...
    points = alt.Chart(cross_df).mark_point(color='red', size=200, filled=True).encode(x='年份',y='花費',tooltip=['年份', '花費'])
